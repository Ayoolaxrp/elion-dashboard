  ...
  return(
    <div className="min-h-screen bg-[#0A0D14] p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {pipes.length===0?<p className="text-[#6B7280]">No pipelines found.</p>:(
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              {pipes.map(p=>{const idx=STAGES.findIndex(s=>s.key===p.current_stage);const c=p.clients;return(
                <button key={p.id} onClick={()=>{setSel(p);setWa(null);}} className={"w-full text-left p-4 rounded-xl border "+(sel?.id===p.id?"border-[#4F7CFF] bg-[#4F7CFF]/5":"border-[#1F2937] bg-[#11161F]")}>
                  <div className="flex items-center justify-between mb-1"><span className="text-sm font-medium text-white">{c?.company_name||"Unknown"}</span><span className="text-xs px-2 py-0.5 rounded bg-[#4F7CFF]/20 text-[#4F7CFF]">{p.current_stage}</span></div>
                  <p className="text-xs text-[#6B7280]">{c?.contact_name} - {c?.email}</p>
                  <div className="flex gap-0.5 mt-2">{STAGES.map((s,i)=><div key={s.key} className={"h-1 flex-1 rounded-full "+(i<idx?"bg-[#10B981]":i===idx?"bg-[#4F7CFF]":"bg-[#1F2937]")} />)}</div>
                </button>);})}
            </div>
            <div className="lg:col-span-2">
              {sel?(
                <div className="space-y-6">
                  <div className="p-6 rounded-xl border border-[#1F2937] bg-[#11161F]">
                    <div className="flex items-center justify-between mb-4"><div><h2 className="text-lg font-bold text-white">{sel.clients?.company_name}</h2><p className="text-sm text-[#6B7280]">{sel.clients?.contact_name} - {sel.clients?.email}</p></div><button onClick={()=>advance(sel.id)} disabled={busy===sel.id} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#4F7CFF] text-white text-sm font-semibold disabled:opacity-50">{busy===sel.id?<Loader2 className="w-4 h-4 animate-spin"/>:<ChevronRight className="w-4 h-4"/>}Advance</button></div>
                    {STAGES.map((s,i)=>{const idx=STAGES.findIndex(st=>st.key===sel.current_stage);const done=i<idx;const cur=i===idx;return(
                      <div key={s.key} className="flex items-center gap-3 py-2"><div className={"w-7 h-7 rounded-full flex items-center justify-center "+(done?"bg-[#10B981]/20":cur?"bg-[#4F7CFF]/20":"bg-[#0A0D14]")}>{done?<CheckCircle className="w-4 h-4 text-[#10B981]"/>:cur?<div className="w-2 h-2 rounded-full bg-[#4F7CFF]"/>:<Circle className="w-4 h-4 text-[#1F2937]"/>}</div><span className={"text-sm "+(cur?"text-white font-medium":done?"text-[#9CA3AF]":"text-[#4B5563]")}>{s.label}</span>{done&&<span className="text-xs text-[#10B981]">Done</span>}{cur&&<span className="text-xs text-[#4F7CFF]">Current</span>}</div>);})}
                  </div>
                  <div className="p-6 rounded-xl border border-[#1F2937] bg-[#11161F]"><h3 className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-4">Actions</h3>
                    {sel.current_stage==="welcome"&&<div className="space-y-3"><p className="text-sm text-[#9CA3AF]">Send welcome email to {sel.clients?.contact_name}.</p><button onClick={()=>sendEmail(sel.id,"welcome")} disabled={!!busy} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#10B981] text-white text-sm font-semibold disabled:opacity-50"><Mail className="w-4 h-4"/>Send Welcome Email</button>{sel.welcome_email_sent&&<p className="text-xs text-[#10B981]">Sent</p>}</div>}
                    ...
                    {sel.current_stage==="handover"&&<div className="space-y-3"><button onClick={()=>sendEmail(sel.id,"completion",{automationName:"Lead Response",connectedSystems:["WhatsApp","Email","Calendar"],workflowDescription:"Automated lead capture, qualification, response, follow-up, and booking."})} disabled={!!busy} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#4F7CFF] text-white text-sm font-semibold disabled:opacity-50"><Send className="w-4 h-4"/>Send Completion Email</button></div>}
                  </div>
              ...
              ):<p className="text-[#6B7280]">Select a client.</p>}
            </div>
          </div>
        )}
      </div>
