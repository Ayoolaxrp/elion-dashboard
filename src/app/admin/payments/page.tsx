  ...
  return (
    <div className="max-w-5xl p-6">
      ...
      <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-xl overflow-hidden">
        <div className="grid grid-cols-6 gap-4 px-5 py-3 border-b border-[var(--color-border)] text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">
          <span>Client</span>
          <span>Amount</span>
          <span>Method</span>
          <span>Reference</span>
        ...
        {allPayments.map(p => {
          ...
          const Icon = sc.icon;
          return (
            <div key={p.id} className="grid grid-cols-6 gap-4 px-5 py-4 border-b border-[var(--color-border)]/50 text-sm items-center">
              <div>
