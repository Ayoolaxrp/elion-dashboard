...
function ProposalPreview({ proposal, form }: { proposal: Proposal; form: ProposalForm }) {
  return <article className="proposal-preview space-y-10 py-8 text-[var(--color-text-primary)]">
    <header className="border-b border-[var(--color-border)] pb-8">
      ...
      <p className="mt-2 text-xs text-[var(--color-text-muted)]">{form.country} · {form.timezone} · Valid until {form.valid_until || "to be confirmed"}</p><p className="mt-2 text-xs text-[var(--color-text-muted)]">{form.client_email || "Email to be confirmed"}{form.contact_role ? ` · ${form.contact_role}` : ""}{form.address ? ` · ${form.address}` : ""}</p>
    </header>
