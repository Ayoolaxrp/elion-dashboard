...
<h2>Summary</h2><table><tr><td>Total leaks</td><td><strong>${r.totalLeaks}</strong></td></tr><tr><td>Critical</td><td>${r.criticalLeaks}</td></tr><tr><td>High</td><td>${r.highLeaks}</td></tr><tr><td>Modeled opportunity scenario</td><td><strong>${r.estimatedAnnualSavings}/year</strong><br><small>Scenario only; not a forecast or guarantee.</small></td></tr></table>
<h2>Identified Gaps</h2><table><thead><tr><th>Area</th><th>Severity</th><th>What we found</th><th>Modeled scenario</th></tr></thead><tbody>${r.leaks.map((l) => `<tr><td>${l.area}</td><td><span class="severity sev-${l.severity}">${l.severity}</span></td><td>${l.description.slice(0, 160)}${l.description.length > 160 ? "..." : ""}</td><td>${l.estimatedSavings}</td></tr>`).join("")}</tbody></table>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-8">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <div className="mt-5 rounded-xl border border-[var(--color-border)]/60 bg-[var(--color-surface)] px-5 py-3.5 inline-flex flex-col sm:flex-row items-center gap-1 sm:gap-3">
                        ...
                        <span className="text-[11px] text-[var(--color-text-muted)]">modeled opportunity scenario, not a forecast</span>
                      </div>
                    </div>
                  </div>
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
                                    ...
                                    <div className="flex items-center gap-2.5 shrink-0">
                                      ...
                                      <span className="text-[10px] font-medium text-[var(--color-text-muted)]">Modeled scenario: {leak.estimatedSavings}</span>
                            ...
                            {isExpanded && (
                              <div className="px-5 pb-5 ml-5 border-t border-[var(--color-border)]/60">
                                <div className="mt-4 space-y-3.5">
                                  <p className="text-[11px] text-[var(--color-text-muted)]">Any financial figure shown here is a modeled scenario based on assumptions, not a promised result.</p>
