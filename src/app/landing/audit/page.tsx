    ...
    <div className="min-h-screen">
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight leading-tight mb-6">
            Find out where your business<br /><span className="text-indigo-400">is leaking time and money.</span>
          </h1>
      ...
      <section id="cta" className="py-20 px-6 border-t border-zinc-800/50">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-zinc-400 mb-8">Fill in your details and we will run the audit instantly. Results shown on screen.</p>
            ...
            {result?.success && result.data ? (() => {
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-sm text-zinc-400 mt-1">Based on {company}</p>
                </div>

                {totalSavings != null && (
                  <div className="text-center p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    <p className="text-sm text-emerald-400 font-medium">Estimated Annual Savings</p>
                    <p className="text-2xl font-bold text-emerald-400 mt-1">NGN {String(totalSavings)}</p>
                  </div>
                )}
                ...
                {leaks.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {leaks.map((leak, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-zinc-800/30 border border-zinc-800/50">
                        <div className="flex items-center gap-2">
                          <AlertTriangle className={`w-4 h-4 ${String(leak.severity) === "critical" ? "text-red-400" : String(leak.severity) === "high" ? "text-amber-400" : "text-zinc-400"}`} />
                          <span className="text-sm text-zinc-300">{String(leak.area)}</span>
                        </div>
                ...
                <button onClick={() => { setResult(null); setForm({ company_name: "", name: "", email: "", industry: "" }); }} className="w-full px-6 py-3 rounded-lg border border-zinc-700 text-sm text-zinc-300 hover:bg-zinc-800 transition-colors">
