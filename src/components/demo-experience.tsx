    ...
    <div className="mx-auto max-w-5xl">
        ...
        <section aria-labelledby="industry-heading">
          ...
          <div className="mt-8 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            {INDUSTRIES.map((value) => (
              <button key={value} type="button" onClick={() => { setIndustry(value); setStage("problem"); setError(""); }} className="group border-t border-[var(--color-border)] bg-[var(--color-surface)] p-5 text-left transition-colors hover:border-[var(--color-accent-bright)] hover:bg-[var(--color-surface-raised)]">
      ...
      {scenario && stage === "conversation" && (
        <section aria-labelledby="conversation-heading" className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
          <div>
            ...
            <button type="button" onClick={() => setStage("finding")} className="public-primary mt-8 px-5 py-3 text-sm">Show the operational finding <ArrowRight className="h-4 w-4" /></button>
          </div>
      ...
      {scenario && stage === "finding" && (
        <section aria-labelledby="finding-heading">
          ...
          <button type="button" onClick={() => setStage("capture")} className="public-primary mt-8 px-5 py-3 text-sm">See how this could fit your business <ArrowRight className="h-4 w-4" /></button>
        </section>
      )}
      ...
      {scenario && stage === "capture" && (
        <section className="max-w-2xl" aria-labelledby="capture-heading">
          ...
          <form onSubmit={submitLead} className="mt-8 space-y-5 border-y border-[var(--color-border)]/60 py-7">
            ...
            <button type="submit" disabled={submitting} className="public-primary w-full px-5 py-3.5 text-sm disabled:cursor-not-allowed disabled:opacity-50">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{submitting ? "Sending request..." : "Ask ELION to review my business"}</button>
